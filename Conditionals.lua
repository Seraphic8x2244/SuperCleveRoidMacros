...
function CleveRoids.IsValidTarget(target, help)
    ...
    if target == "mouseover" then
        effectiveTarget = CleveRoids.ResolveMouseoverUnit()
        if not effectiveTarget then
            return false
        end
    end

    if not UnitExists(effectiveTarget) or not CleveRoids.CheckHelp(effectiveTarget, help) then
        return false
    end
    return true
end
